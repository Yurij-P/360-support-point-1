...
class ImprovementService:
    def create_improvement_action(
        self, title: str, priority: int, deadline: date | None = None
    ) -> ImprovementAction:
    ...
    def identify_overdue_actions(
        self, plan: ImprovementPlan, today: date | None = None
    ) -> list[ImprovementAction]:
        current = today or date.today()
        return [
            action
            for action in plan.actions
            if action.deadline and action.deadline < current and action.status != "completed"
        ]
