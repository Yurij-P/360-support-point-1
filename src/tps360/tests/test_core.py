...
from tps360.core.services import (
    EvaluationService,
    ImprovementService,
    PreparednessService,
    RiskService,
...
def test_community_valid(population):
    assert (
        Community(
            name="A", code=str(population), oblast="O", population=population, area_km2=1
        ).population
        == population
    )
...
def test_community_rejects_negative_population():
    with pytest.raises(ValueError):
        Community(name="A", code="x", oblast="O", population=-1, area_km2=1)


@pytest.mark.parametrize(
    "score,level",
    [(0, "reactive"), (20, "basic"), (40, "managed"), (60, "integrated"), (80, "resilient")],
)
def test_cpmm(score, level):
    assert PreparednessService().determine_maturity_level(score).value == level
...
def test_simulation_lifecycle_and_decision():
    ...
    svc.record_decision(
        sim,
        Decision(
            simulation_id=sim.id, actor="a", description="d", rationale="r", selected_action="x"
        ),
    )
    assert len(sim.decisions) == 1
    svc.complete_simulation(sim)
    assert sim.status is LifecycleStatus.COMPLETED
...
def test_inject_delivery_requires_active():
    ...
    with pytest.raises(DomainRuleViolation):
        svc.deliver_inject(sim, inject)
    svc.start_simulation(sim)
    assert svc.deliver_inject(sim, inject).delivered_at is not None


def test_evaluation_and_improvement():
    ev = Evaluation(
        simulation_id=uuid4(),
        criteria_results={"c": 80},
        capability_scores={"x": 60},
